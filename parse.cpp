...
int ParseCountParams(struct ParseState *Parser)
{
    ...
    if (Token != TokenCloseBracket && Token != TokenEOF)
    {
        ...
        ParamCount++;
        while ((Token = LexGetToken(Parser, nullptr, true)) != TokenCloseBracket && Token != TokenEOF)
        {
            if (Token == TokenComma)
                ParamCount++;
        }
...
struct Value *ParseFunctionDefinition(struct ParseState *Parser, struct ValueType *ReturnType, char *Identifier)
{
    ...
    FuncValue = VariableAllocValueAndData(pc, Parser, sizeof(struct FuncDef) + sizeof(struct ValueType *) * ParamCount + sizeof(const char *) * ParamCount, false, nullptr, true);
    ...
    for (ParamCount = 0; ParamCount < FuncValue->Val->FuncDef.NumParams; ParamCount++)
    {
        ...
        {
            /* add a parameter */
            TypeParse(&ParamParser, &ParamType, &ParamIdentifier, nullptr);
    ...
    if (Token == TokenSemicolon)
        LexGetToken(Parser, nullptr, true);    /* it's a prototype, absorb the trailing semicolon */
    else
    {
...
int ParseDeclaration(struct ParseState *Parser, enum LexToken Token)
{
    ...
    struct Value *NewVariable = nullptr;
    int IsStatic = false;
...
void ParseMacroDefinition(struct ParseState *Parser)
{
    ...
    if (LexRawPeekToken(Parser) == TokenOpenMacroBracket)
    {
        ...
        MacroValue = VariableAllocValueAndData(Parser->pc, Parser, sizeof(struct MacroDef) + sizeof(const char *) * NumParams, false, nullptr, true);
    ...
    else
    {
        ...
        MacroValue = VariableAllocValueAndData(Parser->pc, Parser, sizeof(struct MacroDef), false, nullptr, true);
...
void ParseTypedef(struct ParseState *Parser)
{
    ...
    TypeParse(Parser, &Typ, &TypeName, nullptr);
...
void PicocParseInteractiveNoStartPrompt(Picoc *pc, int EnableDebugger)
{
    ...
    LexInitParser(&Parser, pc, nullptr, nullptr, pc->StrEmpty, true, EnableDebugger);
